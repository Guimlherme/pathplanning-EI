...
def preprocessing_two_lines(image,save_photos=False):
  ...
  if cv2.contourArea(contours[0])>MINIMUM_AREA:
    ...
    if cv2.contourArea(contours[1])>MINIMUM_AREA:
      cx_mean = []
      cy_mean = []
      for i in range(2):
      ...
      if cx > (width/2):
        cx_mean = int(cx - (DISTANCE_BETWEEN_LINES/2))
        cy_mean = cy
      else:
        cx_mean = int(cx + (DISTANCE_BETWEEN_LINES/2))
        cy_mean = cy
...
def preprocessing_image(image,save_photos=False):
  if ONE_LINE_MAP:
    preprocessing_one_line(image,save_photos)
  else:
    preprocessing_two_lines(image,save_photos)
